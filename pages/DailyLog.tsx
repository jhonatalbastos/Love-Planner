                        ...
                        return (
                             <div key={goal.id} className="flex items-center gap-3 bg-white dark:bg-card-dark p-3 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm transition-all">
                                <button 
                                    ...
                                    disabled={isLocked}
                                    className={`flex items-center justify-center rounded-lg shrink-0 size-8 border-2 transition-all duration-200 active:scale-95 ${
                                        isVisuallyDone
                   ...
                   return (
                     <div key={agreement.id} className="flex items-center gap-3 bg-white dark:bg-card-dark p-3 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm transition-all">
                        <button 
                            ...
                            disabled={isLocked}
                            className={`flex items-center justify-center rounded-full shrink-0 size-6 border-2 transition-all duration-200 ${
                                isCompleted 
          ...
          <div className="flex flex-wrap gap-2.5">
            {preferences.partnerActionOptions.map(option => (
              <button 
                ...
                disabled={isLocked}
                className={`flex items-center gap-2 pl-3 pr-4 py-2.5 rounded-xl transition-all active:scale-95 border ${
                  selectedPartnerActions.includes(option.label) 
             ...
             <div className="grid grid-cols-5 gap-1">
                 {LOVE_LANGUAGES.map(lang => (
                     <button
                        ...
                        disabled={isLocked}
                        className={`flex flex-col items-center justify-center p-1 rounded-lg transition-all w-full ${
                            partnerLoveLanguages.includes(lang.label)
          ...
          <div className="flex flex-wrap gap-2.5">
             {preferences.myActionOptions.map(option => (
              <button 
                ...
                disabled={isLocked}
                className={`flex items-center gap-2 pl-3 pr-4 py-2.5 rounded-xl transition-all active:scale-95 border ${
                  selectedMyActions.includes(option.label) 
                    ? 'bg-primary text-white shadow-soft border-primary' 
                    : 'bg-card-light dark:bg-card-dark border-border-light dark:border-border-dark shadow-sm hover:border-primary/50'
                }`}
              >
             ...
             <div className="grid grid-cols-5 gap-1">
                 {LOVE_LANGUAGES.map(lang => (
                     <button
                        ...
                        disabled={isLocked}
                        className={`flex flex-col items-center justify-center p-1 rounded-lg transition-all w-full ${
                            myLoveLanguages.includes(lang.label)
                            ? `bg-white dark:bg-card-dark shadow-sm border border-primary/20 ${lang.color}`
                            : 'opacity-50 grayscale hover:opacity-100 hover:grayscale-0'
                        }`}
                     >
